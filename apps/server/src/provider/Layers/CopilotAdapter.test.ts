...
it.layer(CopilotAdapterTestLayer)("CopilotAdapterLive", (it) => {
  it.effect("stops sessions and settles pending approvals when its instance scope closes", () =>
    Effect.gen(function* () {
      ...
      const { adapter, pendingPermission } = yield* Effect.scoped(
        Effect.gen(function* () {
          const adapter = yield* makeCopilotAdapter(decodeCopilotSettings({}), {
            instanceId: ProviderInstanceId.make("copilot-personal"),
            nativeEventLogger,
          });
          ...
          NodeAssert.equal(session.providerInstanceId, "copilot-personal");
          const config = runtimeMock.state.createSessionConfigs.at(-1);
          NodeAssert.ok(config?.onPermissionRequest);
          const pendingPermission = Promise.resolve(
            config.onPermissionRequest({ kind: "shell" } as PermissionRequest, {
              sessionId: session.threadId,
            }),
          );
          return { adapter, pendingPermission };
        }),
      );
      ...
      NodeAssert.deepStrictEqual(yield* Effect.promise(() => pendingPermission), {
        kind: "reject",
      });
    }),
  );

  it.effect("cleans up its client when session startup is interrupted", () =>
    Effect.gen(function* () {
      const started = Promise.withResolvers<void>();
      runtimeMock.state.createSessionImpl = async () => {
        started.resolve();
        return new Promise<CopilotSession>(() => undefined);
      };
      const adapter = yield* CopilotAdapter;
      const threadId = asThreadId("copilot-interrupted-startup");
  ...
  it.effect("launches a local session with the upstream device environment", () =>
    Effect.gen(function* () {
      const adapter = yield* CopilotAdapter;
      const threadId = asThreadId("copilot-device-environment");
      setMcpProviderSession(threadId, "device-test-token");
      const mcpSession = McpProviderSession.readMcpProviderSession(threadId);
      NodeAssert.ok(mcpSession);
      McpProviderSession.setMcpProviderSession({
        ...mcpSession,
        ...
        agentDeviceEnvironment: {
          PATH: "/device-shim",
          PATH_SEPARATOR: ":",
          AGENT_DEVICE_SERVER: "http://127.0.0.1:43210",
        },
      });

      yield* adapter.startSession({ threadId, runtimeMode: "approval-required" });
      const env = vi.mocked(createCopilotClient).mock.calls.at(-1)?.[0].env;
      const basePath = process.env.PATH ?? process.env.Path;
      NodeAssert.equal(env?.AGENT_DEVICE_SERVER, "http://127.0.0.1:43210");
      NodeAssert.equal(env?.PATH, basePath ? `/device-shim:${basePath}` : "/device-shim");
      NodeAssert.equal(env?.PATH_SEPARATOR, undefined);
  ...
  it.effect("does not treat read-only git diff output as an edit", () =>
    Effect.gen(function* () {
      const adapter = yield* CopilotAdapter;
      const threadId = asThreadId("copilot-read-only-diff");
      yield* adapter.startSession({ threadId, runtimeMode: "full-access" });
      const turn = yield* adapter.sendTurn({ threadId, input: "review the current diff" });
        ...
        Stream.runForEach((event) =>
          Effect.gen(function* () {
            ...
            if (event.type === "item.completed") {
              yield* Deferred.succeed(toolCompleted, undefined);
      ...
      NodeAssert.ok(config?.onEvent);
      const timestamp = yield* nowIso;
      const base = { timestamp, parentId: null };
      config.onEvent({
        ...base,
        id: "read-only-diff-start",
        type: "assistant.turn_start",
        data: { turnId: "sdk-read-only-diff" },
      } as SessionEvent);
      config.onEvent({
        ...base,
        id: "read-only-diff-tool-start",
        type: "tool.execution_start",
        data: {
          toolCallId: "read-only-diff-tool",
          toolName: "bash",
      ...
      NodeAssert.equal(
        events.some((event) => event.type === "turn.diff.updated"),
        false,
      );
      NodeAssert.ok(
        events.some(
  ...
  it.effect("emits and terminally fails non-eligible Copilot quota errors", () =>
    Effect.gen(function* () {
      ...
      if (rateLimitEvent?.type === "account.rate-limits.updated") {
        NodeAssert.deepStrictEqual(rateLimitEvent.payload.limits, { windows: [] });
        NodeAssert.equal(rateLimitEvent.providerInstanceId, COPILOT_INSTANCE_ID);
        const rawEvent = rateLimitEvent.raw?.payload as SessionEvent | undefined;
        NodeAssert.equal(rawEvent?.type, "session.error");
        if (rawEvent?.type === "session.error") {
          NodeAssert.deepStrictEqual(rawEvent.data, {
            errorType: "quota",
            errorCode: "quota_exceeded",
            message: "Copilot quota exceeded",
            eligibleForAutoSwitch: false,
