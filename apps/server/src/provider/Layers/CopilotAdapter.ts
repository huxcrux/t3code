...
function completedToolDiffText(
  ...
  return hasUnifiedDiffShape(diffCandidate) ? diffCandidate : undefined;
}
      ...
      case "session.error": {
        ...
        if (isAccountLimitError) {
          await emitAsync({
            ...
            payload: {
              // SDK errors name the limit but contain no usage windows. Keep
              // published limits unchanged and retain diagnostics in `raw`.
              limits: { windows: [] },
            },
          });
  ...
  const startSession: CopilotAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const baseSessionConfig = {
        ...
        systemMessage: {
          mode: "append",
          content: buildRuntimeInstructions({ harness: "GitHub Copilot" }),
        },
