...
it.layer(CopilotTextGenerationTestLayer)("CopilotTextGeneration", (it) => {
  ...
  it.effect("normalizes generated commit, branch, and change request text", () =>
    Effect.gen(function* () {
      const textGeneration = yield* makeCopilotTextGeneration(defaultCopilotSettings);
      const modelSelection = createModelSelection(ProviderInstanceId.make("copilot"), "gpt-4.1");
      runtimeMock.state.responseContent = {
        subject: "  Fix reconnect startup.\nExtra subject text",
        body: "  Preserve the active turn.  ",
        branch: "Fix Reconnect Startup",
      };
      ...
      expect(commit).toEqual({
        subject: "Fix reconnect startup",
        body: "Preserve the active turn.",
        branch: "feature/fix-reconnect-startup",
      });

      const branch = yield* textGeneration.generateBranchName({
        cwd: process.cwd(),
        message: "Fix reconnect startup",
        modelSelection,
      });
      expect(branch).toEqual({ branch: "fix-reconnect-startup" });

      runtimeMock.state.responseContent = {
        title: "  Fix reconnect startup\nExtra title text",
        body: "  Preserve the active turn.  ",
      };
      const changeRequest = yield* textGeneration.generatePrContent({
        cwd: process.cwd(),
        baseBranch: "main",
        headBranch: branch.branch,
        commitSummary: commit.subject,
        diffSummary: "M app.ts",
        diffPatch: "+fix()",
        modelSelection,
      });
