...
export function usePreferredEditor(availableEditors: ReadonlyArray<EditorId>) {
  ...
  const effectiveEditor = useMemo(() => {
    if (
      primaryEditor &&
      EDITORS.some((editor) => editor.id === primaryEditor) &&
      availableEditors.includes(primaryEditor as EditorId)
    ) {
      return primaryEditor as EditorId;
    }
    if (lastEditor && availableEditors.includes(lastEditor)) return lastEditor;
    return EDITORS.find((editor) => availableEditors.includes(editor.id))?.id ?? null;
  }, [availableEditors, lastEditor, primaryEditor]);

  const setPreferredEditor = useCallback(
    (editor: EditorId) => {
      setLastEditor(editor);
      updateClientSettings({ primaryEditor: editor });
    },
    [setLastEditor, updateClientSettings],
  );

  useEffect(() => {
    if (primaryEditor === null && lastEditor !== null) {
      updateClientSettings({ primaryEditor: lastEditor });
    }
  }, [lastEditor, primaryEditor, updateClientSettings]);
...
export function resolveAndPersistPreferredEditor(
  ...
  const configured = getClientSettings().primaryEditor;
  if (
    configured &&
    EDITORS.some((editor) => editor.id === configured) &&
    availableEditorIds.has(configured as EditorId)
  ) {
    return configured as EditorId;
  }
